...
import torch
import itertools
#import utils # fuse not being use, ref didnt include script so no problem either ways
...
def b16(n, activation, resolution=256):
...
class LeViT_UNet_128s(torch.nn.Module):
    ...
    def __init__(self, img_size=224, ## aping: should change
        ...
        self.cnn_b1 = torch.nn.Sequential(
            Conv2d_BN(in_chans, n // 8, 3, 2, 1, resolution=img_size), activation())
...
if __name__ == '__main__':
    for name in specification:
        ...
        net = Build_LeViT_UNet_128s(num_classes=1, pretrained=True)
        net.eval()
        net(torch.randn(1, 3, 224, 224)) #NCHW
        # print(net)
        print(name,
              net.FLOPS, 'FLOPs',
